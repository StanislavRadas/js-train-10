 ...
 * Крім того, вона перетворює значення числових властивостей на рядки 
...
function customObjectFromEntries(entries) {
  if (Array.isArray(entries)) {
    const newArr = entries.map(entry => {
      let key = entry[0];
      let value = entry[1];
      if (typeof value === 'number') {
        value = value.toString();
    ...
    const result = Object.fromEntries(newArr);
    return result;
  } else {
...
function filterObjectsByKey(arr, key) {
  if (Array.isArray(arr)) {
    let newArr = arr.filter(elem => elem = elem.hasOwnProperty(key));
    return newArr
  } else {
    return [];
  }
...
function checkAndPreventExtensions(obj) {
  if (typeof obj === 'object') {
    if (Object.isExtensible(obj) === true) {
      ...
      return Object.isExtensible(result);
    }
  } else {
    return false;
  }
...
function getObjectValuesSum(obj) {
  if (typeof obj === 'object') {
    ...
    let result = objValue.reduce((accumulator, currentValue) => accumulator + currentValue);
    return result;
  } else {
    return 0;
  }
